...
class AdminUser(db.Model):
    ...
    updatetime = Column(DateTime, nullable=True,default=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))   #存储变更时间
...
class ProdInfo(db.Model):
    ...
    def __count_card__(self,prod_name):
        ...
        if count > 10:
            ...
            #再次统计
            if Card.query.filter_by(prod_name = prod_name,reuse = 1).first():
                return '充足'
            return '少量'
        else:
            return '少量' 
...
class Order(db.Model):
    ...
    updatetime = Column(DateTime, nullable=True,default=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))  #交易时间
...
class Message(db.Model):
    ...
    updatetime = Column(Text, nullable=True,default=datetime.now().strftime('%Y-%m-%d %H:%M:%S'))  #
