...
def init_db(update=False):
    ...
    db.session.add(Payment('虎皮椒支付宝','支付宝',"{'appid':'XXXXXX','AppSecret':'YYYYY'}",'xunhupay.com 个人接口0.38%+1~2%',False))
    db.session.add(Payment('虎皮椒微信','微信支付',"{'appid':'XXXXXX','AppSecret':'YYYYY'}",'xunhupay.com 个人接口0.38~0.6%+1~2%',False))
    ...
    db.session.add(Payment('Mugglepay','Mugglepay',"{'TOKEN':'xxxxxx','Currency':'CNY'}",'mugglepay.com全球综合收款系统 高费率',False))
